...
pub struct H2ServerConfig<L: HyperListener> {
    listener: Option<L>,
    endpoint: String,
    extra_headers: Vec<super::super::utils::HeaderPair>,
    frame_limit: usize,
...
impl<L: HyperListener> H2ServerConfig<L> {
    pub fn new(listener: L, endpoint: impl Into<String>) -> Self {
        Self {
            listener: Some(listener),
            endpoint: endpoint.into(),
            extra_headers: Vec::new(),
            frame_limit: super::super::consts::DEFAULT_FRAME_LIMIT,
    ...
    pub fn with_extra_header(
        ...
        self.extra_headers
            .push(super::super::utils::HeaderPair::new(name, value));
        self
    }
...
impl<L: HyperListener> ServerConfig<H2ServerTransport<L>> for H2ServerConfig<L> {
    fn serve<T: Send + Sync + 'static, TB: ServiceDispatch<T, H2ServerChannel>>(
        mut self,
        factory: impl Fn() -> T + Send + Sync + 'static,
    ) -> Self {
        self.services.insert(
            TB::SERVICE_SCHEMA.name.to_string(),
            HxRegisteredService::capture::<T, H2SessionBridge<TB>>(factory),
        );
        self
    }
}

struct H2ServerTransportInner<L: HyperListener> {
    listener: L,
    consts: HxServerConsts,
    _marker: PhantomData<fn() -> L>,
}

impl<L: HyperListener> H2ServerTransportInner<L> {
    ...
    async fn run_connection(this: usize, io: L::Io) -> anyhow::Result<()> {
        ...
        let result = Builder::new(join.as_ref().refer())
            .serve_connection(io, serve)
            .await;
        join.as_ref().join().await;
        result.context("h2 server connection failed")?;
        Ok(())
    }
...
pub struct H2ServerTransport<L: HyperListener> {
    inner: Option<Box<H2ServerTransportInner<L>>>,
    accept_task: Option<JoinHandle<anyhow::Result<()>>>,
}

impl<L: HyperListener> ServerTransport for H2ServerTransport<L> {
    type Config = H2ServerConfig<L>;
    type Channel = H2ServerChannel;
    ...
    async fn configure(mut config: Self::Config) -> anyhow::Result<Self> {
        ...
        let listener = config
            .listener
            .take()
            .ok_or_else(|| anyhow!("h2 server config listener already consumed"))?;
        let endpoint = config.endpoint;

        let inner = Box::new(H2ServerTransportInner {
            listener,
        ...
        let opaque = inner.as_ref() as *const H2ServerTransportInner<L> as usize;
        let accept_task = tokio::spawn(async move {
            let inner = unsafe { &*(opaque as *const H2ServerTransportInner<L>) };
            inner.run().await
        });
    ...
    async fn complete(mut self) -> anyhow::Result<()> {
        ...
        match result {
            Ok(result) => result,
            Err(_) => Err(anyhow!("h2 server accept task failed")),
        }
    }
}

impl<L: HyperListener> Drop for H2ServerTransport<L> {
    fn drop(&mut self) {
...
pub struct H2ServerChannel(HxServerChannel);

impl ServerChannel for H2ServerChannel {
...
struct H2SessionBridge<TB>(PhantomData<TB>);

impl<TB: ServiceFactory> ServiceFactory for H2SessionBridge<TB> {
    const SERVICE_SCHEMA: crate::ServiceSchema<'static> = TB::SERVICE_SCHEMA;
}

impl<T, TB> ServiceDispatch<T, HxServerChannel> for H2SessionBridge<TB>
where
    T: Send + Sync + 'static,
    TB: ServiceDispatch<T, H2ServerChannel>,
{
    async fn dispatch(handler: &T, method_id: u32, channel: HxServerChannel) -> bool {
        TB::dispatch(handler, method_id, H2ServerChannel(channel)).await
    }
}
